...
public class UserInputService {
    ...
    public RegisterFile requestRegisterFile() throws IOException {
        ...
        System.out.println(" Breslau Land:  25) Geburtsregister   26) Heiratsregister   27) Sterberegister ");
    ...
    public void requestUserInput(final List<Register> registerList)
            throws IOException {
        noScansMode = "1".equals(readLine(
                "No scans mode? No Date input, no online/offline input (1 for enable, empty for regular mode): "));
        while (true) {
            ...
            while (!inputDone) {
                final String url = readLine("URL (empty if missing, + if last one +1): ");
                final boolean isMissing = url.isBlank();
                final boolean isPlus = url.equals("+");
                register.setMissing(isMissing);
                if (!isMissing) {
                    String urlToUse = url;
                    if (isPlus) {
                        // Increment the number at the end of the last URL
                        int lastSlashIndex = lastUrl.lastIndexOf('/');
                        if (lastSlashIndex != -1 && lastSlashIndex < lastUrl.length() - 1) {
                            String numberStr = lastUrl.substring(lastSlashIndex + 1);
                            try {
                                int number = Integer.parseInt(numberStr);
                                number++;
                                urlToUse = lastUrl.substring(0, lastSlashIndex + 1) + number;
                            } catch (final NumberFormatException e) {
                                System.out.println("Could not parse number from last URL");
                                continue;
                            }
                        } else {
                            System.out.println("Invalid last URL format");
                            continue;
                        }
                    }
                    try {
                        register.setUrl(new URI(urlToUse).toURL());
                        inputDone = true;
